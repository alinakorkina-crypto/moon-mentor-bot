"""Experimental one-call Reading Engine v3.4 for Moon Mentor.
...
SPREAD_INSTRUCTIONS = {
    ...
    "love": (
        ...
        "человека «таким, какой он есть», принимать текущую динамику, снижать ожидания "
        "или подстраиваться под предполагаемый ритм другого человека. Условие прямого "
        "ответа и критерий должны проверять, поддерживают ли контакт двое. Если вопрос "
        "описывает исчезновения, но не инициативу обеих сторон, не делайте вывод об "
        "односторонности, отсутствии взаимности или устойчивой вовлечённости: предложите "
        "пользователю проверить это как критерий."
    ),
    "career": (
        ...
        "опасностью, тайной или препятствием. Если обязанности расплывчаты, рекомендуйте "
        "принимать решение после письменной конкретизации обязанностей, полномочий и "
        "условий, а не соглашаться сначала и прояснять роль потом. Не предполагайте, что "
        "пользователь сможет сам сформировать роль или условия. Не обещайте успех, рост "
        "или повышение и не называйте действия пользователя «ключом к успеху»."
    ),
    "personal_question": (
...
Напишите четыре коротких абзаца без заголовков:
1. Прямой ответ, условие этого ответа и центральная дилемма.
2. Единый рисунок сочетания, главное противоречие или усиление и роль позиций.
   Сначала сформулируйте общий рисунок, затем используйте названия карт только как
   аргументы внутри этой мысли. Не двигайтесь по картам по очереди.
3. Один наблюдаемый критерий и пояснение, как проверить его в уже существующих
   действиях, словах или условиях.
4. Один конкретный следующий шаг и один точный вопрос для размышления.

Общий объём четырёх абзацев — 180–220 слов. Естественность и точность важнее
механического количества предложений.
Перед выдачей молча проверьте четыре абзаца и общий объём.
...
- Не начинай словами «карты показывают», «расклад говорит» или названием карты.
- Выдели одну центральную тему и главное противоречие или усиление сочетания.
- Сначала сформулируй общий рисунок сочетания, затем вплети карты в аргументацию.
- Свяжи карты в одну линию. Не создавай отдельное толкование для каждой карты.
- Не используй механические конструкции «позиция представлена картой»,
  «карта указывает», «карта подчёркивает», «карта подталкивает» и их аналоги.
...
- Следующий шаг должен быть возможен на основании вопроса пользователя.
- Не придумывай встречу, совместное занятие, сообщение, разговор, эксперимент или
  проверку, если в вопросе нет основания считать такое действие возможным.
- Если контекста для действия мало, предложи определить личную границу, уточнить
  недостающий факт или наблюдать уже существующий рисунок ситуации.
...
- Не усиливай слова пользователя: «пропадает» не превращай в «полный уход» или
  «глубокую дистанцию», а повторяемость не называй неизменной или природной.
- Не делай вывод об отсутствии взаимности, односторонности или устойчивой
  вовлечённости, если вопрос не описывает инициативу и действия обеих сторон.
- Недостаток информации не называй скрытой информацией, тайной или препятствием.
...
def inspect_reading_v3_answer(answer: str, spread_type: str) -> dict[str, Any]:
    ...
    min_words, max_words = (
        (90, 130) if spread_type == "daily_card" else (180, 220)
    )
    issues: list[str] = []
    if not min_words <= words <= max_words:
        issues.append(f"word_count:{words}")
    expected_paragraphs = 3 if spread_type == "daily_card" else 4
