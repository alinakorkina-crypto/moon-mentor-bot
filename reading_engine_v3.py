"""Experimental one-call Reading Engine v3.3 for Moon Mentor.
...
SPREAD_INSTRUCTIONS = {
    ...
    "love": (
        ...
        "ответа и критерий должны проверять, поддерживают ли контакт двое."
    ),
    "career": (
        ...
        "и не называйте действия пользователя «ключом к успеху»."
    ),
    "personal_question": (
...
Напишите ровно четыре коротких абзаца без заголовков и ровно 13 предложений:
1. Три предложения: прямой ответ, условие этого ответа и центральная дилемма.
2. Пять предложений: единый рисунок сочетания, главное противоречие или усиление и
   роль позиций. Сначала сформулируйте общий рисунок, затем используйте названия карт
   как аргументы внутри этой мысли. Не двигайтесь по картам по очереди.
3. Три предложения: один наблюдаемый критерий и пояснение, как проверить его в
   существующих действиях, словах или условиях.
4. Два предложения: один конкретный следующий шаг и один точный вопрос для размышления.

Общий объём четырёх абзацев — строго 180–220 слов.
Перед выдачей молча проверьте: четыре абзаца, 13 предложений и общий объём.
