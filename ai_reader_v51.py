"""Gemini transport with finish diagnostics for Reading Engine v5.1."""
...
def ask_gemini_analysis_v51(prompt: str) -> dict[str, Any]:
    try:
        ...
        print(f"Gemini v5.1 analysis error: {error}")
        return {"text": None, "finish_reason": "ERROR", "usage": None}


def ask_gemini_editor_v51(prompt: str) -> dict[str, Any]:
    try:
        response = client.models.generate_content(
            model=MODEL_NAME,
            contents=prompt,
        ...
        print(f"Gemini v5.1 editor error: {error}")
        return {"text": None, "finish_reason": "ERROR", "usage": None}
