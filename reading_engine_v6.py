...
ROUTE_CONFIG_V6 = {
    "initiative": {
        ...
        "criterion_question_guides": {
            "independent_contact": (
                "Спросите, какое конкретное самостоятельное действие пользователь сочтёт "
                "инициативой. Не называйте его подтверждением интереса или чувств."
            ),
            "sustained_conversation": (
                "Спросите, какое продолжение разговора будет отличаться от случайной реакции."
            ),
            "concrete_invitation": (
                "Спросите, какое конкретное предложение пользователь сочтёт проявлением."
            ),
            "repeated_initiative": (
                "Спросите, какая повторяемость действий покажет устойчивую инициативу."
            ),
        },
    },
    "personal_choice": {
        ...
        "criterion_question_guides": {
            "mutual_initiative": (
                "Спросите, какие наблюдаемые действия покажут, что связь поддерживается "
                "обеими сторонами."
            ),
            "consistency": (
                "Спросите, какая наблюдаемая регулярность контакта необходима пользователю. "
                "Не спрашивайте, насколько комфортно ждать во время пауз."
            ),
            "boundary_response": (
                "Спросите, какая реальная реакция на обозначенную границу поможет сделать выбор."
            ),
            "clear_conditions": (
                "Спросите, какие условия должны стать ясными до принятия решения."
            ),
            "concrete_information": (
                "Спросите, какой конкретной информации не хватает для выбора."
            ),
        },
        "topic_profiles": {
            "love": {
...
final_text:
- ровно 3 коротких абзаца, цель 105–140 слов;
...
- угол размышления связывает вопрос с неопределённостью, границами или условиями
  выбора только там, где это прямо следует из вопроса;
...
- не употребляйте метаязык «психологический фокус ситуации» и не придумывайте
  внутреннее напряжение, вынужденное ожидание, желаемый образ, далёкую перспективу
  или непреодолимый барьер;
- не называйте контакт подтверждением интереса, чувств или намерений;
...
достаточной опоры рассчитывать на самостоятельный первый шаг. Возможность контакта
остаётся открытой, но не становится подтверждённым действием другого человека.
...
случайная реакция. Какое конкретное действие вы сочтёте настоящей инициативой?
""".strip(),
...
ADAPTATION_AND_BLAME_PATTERNS_V6 = (
    ...
    r"\bнасколько\s+комфортн\w*(?:\s+\w+){0,4}\s+(?:ждать|находиться)\b",
)
...
UNSUPPORTED_INFERENCE_PATTERNS_V6 = (
    ...
    ("psychological_focus_meta", r"\bпсихологическ\w*\s+фокус\w*\b"),
    ("inner_tension", r"\bвнутренн\w*\s+напряжени\w*\b"),
    ("forced_waiting", r"\bвынужденн\w*\s+ожидани\w*\b"),
    ("distant_perspective", r"\bдал[её]к\w*\s+перспектив\w*\b"),
    ("desired_image", r"\bжелаем\w*\s+образ\w*\b"),
    ("insurmountable_barrier", r"\bнепреодолим\w*\s+барьер\w*\b"),
    (
        "interest_confirmation",
        r"\bподтверждени\w*(?:\s+\w+){0,2}\s+интерес\w*\b",
    ),
    ("waiting_costs", r"\bзатрат\w*\s+на\s+ожидан\w*\b"),
    ("rare_moments", r"\bредк\w*(?:\s+\w+){0,2}\s+момент\w*\b"),
...
def build_editor_prompt_v6(
    ...
    verified_plan = {
        ...
        "reflection_question_guide": config["criterion_question_guides"][
...
Любое новое объяснение внутреннего состояния, которого нет в вопросе, запрещено.
Верните только JSON.
""".strip()
