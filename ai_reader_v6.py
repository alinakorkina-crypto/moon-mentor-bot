"""Two-pass Gemini caller for Reading Engine v6."""
...
def _result(response: Any) -> dict[str, Any]:
    ...
    return {"text": getattr(response, "text", None), "finish_reason": reason, "usage": usage_dict}


def ask_gemini_analysis_v6(prompt: str) -> dict[str, Any]:
    try:
        return _result(
            client.models.generate_content(
                model=MODEL_NAME, contents=prompt, config=ANALYSIS_CONFIG
            )
        )
    except Exception as error:
        print(f"Gemini v6 analysis error: {error}")
        return {"text": None, "finish_reason": "ERROR", "usage": None}


def ask_gemini_editor_v6(prompt: str) -> dict[str, Any]:
    try:
        return _result(
            client.models.generate_content(
                model=MODEL_NAME, contents=prompt, config=EDITOR_CONFIG
            )
        )
    except Exception as error:
        print(f"Gemini v6 editor error: {error}")
        return {"text": None, "finish_reason": "ERROR", "usage": None}
